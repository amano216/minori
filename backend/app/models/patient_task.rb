...
class PatientTask < ApplicationRecord
  ...
  CATEGORIES = %w[board task].freeze
  ...
  validates :content, presence: true, if: :board?
  validates :task_type, presence: true, inclusion: { in: TASK_TYPES }, if: :task?
  validates :status, presence: true, inclusion: { in: STATUSES }

  scope :boards, -> { where(category: "board") }
