...
class Api::VisitsControllerConcurrencyTest < ActionDispatch::IntegrationTest
  ...
  test "should return 409 when updating with stale lock_version" do
    ...
    # Simulate another user updating the visit
  ...
  test "should return 409 when staff double booking occurs" do
    ...
    # Try to create overlapping visit for same staff
    post api_visits_url,
         params: {
           visit: {
  ...
  test "should return 409 when patient double booking occurs" do
    scheduled_time = 1.day.from_now.change(hour: 14, min: 0)
    other_staff = users(:staff2)
    ...
    # Try to create overlapping visit for same patient with different staff
    post api_visits_url,
         params: {
           visit: {
             patient_id: @patient.id,
    ...
    json = JSON.parse(response.body)
    assert_equal "double_booking", json["error_type"]
    assert_equal "patient", json["conflict_type"]
    assert_includes json["errors"].first, "患者は既に別の訪問が予定されています"
  end
  ...
  test "should allow non-overlapping visits for same staff" do
    ...
    # Create non-overlapping visit (starts after first visit ends)
    post api_visits_url,
         params: {
           visit: {
             patient_id: @patient.id,
             staff_id: @staff.id,
             scheduled_at: scheduled_time + 90.minutes,  # 30 min gap
             duration: 60
           }
         },
         headers: auth_headers(@token),
  ...
  test "should allow creating visit when conflicting visit is cancelled" do
    scheduled_time = 1.day.from_now.change(hour: 10, min: 0)

    # Create cancelled visit
    Visit.create!(
      organization: @organization,
      patient: @patient,
      user: @staff,
    ...
    # Create overlapping visit (should succeed because existing is cancelled)
    post api_visits_url,
         params: {
           visit: {
             patient_id: @patient.id,
  ...
  def generate_token_for(user)
    JWT.encode(
      ...
      Rails.application.credentials.secret_key_base || "test_secret"
    )
  end
end
